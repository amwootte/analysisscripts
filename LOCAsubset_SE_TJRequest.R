...
findfli=function(TMAX,TMIN,lat){
  ...
  if(all(is.na(TMAX)==TRUE)==FALSE & all(is.na(TMIN)==TRUE)==FALSE){
    calcsires = calc_si(TMAX=TMAX,TMIN=TMIN,lat=lat)
    calcsires$FLImat[1,1]
  } else {
    NA
  }
}

findfbi=function(TMAX,TMIN,lat){
...
for(i in 1:length(filesin)){
  ...
  if(tempres=="annual"){
    ...
    for(y in 1:length(years)){
      ...
      if(varout=="frd"){
        tmp1 = apply(testdat[,,idxmon],c(1,2),lastfreeze,startdate=startdate,inputtimes=timeout[idxmon],threshold=0)
      }
      if(varout=="FLI" | varout=="FBI"){
        ...
        data=list(TMAX = testdat[,,idxmon],TMIN=testdat2[,,idxmon],lat=latmat)
        if(varout=="FLI"){
