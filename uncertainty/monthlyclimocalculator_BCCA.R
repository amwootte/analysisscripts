...
files = system(paste("ls /data4/data/DS_proj/BCCA_CMIP5/",var,"/historical/*2005*.nc4",sep=""),intern=TRUE)
...
GCMs = paste(filelist2[,5],filelist2[,6],filelist2[,7],sep="_")

for(i in 1:length(GCMs)){
  
  if(i==1){
  nctest = nc_open(files[1])
    lon = ncvar_get(nctest,"longitude")
    lat = ncvar_get(nctest,"latitude")
  ...
  filesused = system(paste("ls /data4/data/DS_proj/BCCA_CMIP5/",var,"/historical/*",GCMs[i],"*.nc4",sep=""),intern=TRUE)
  fl = do.call("rbind",strsplit(filesused,"/",fixed=TRUE))
  fl2 = do.call("rbind",strsplit(fl[,8],"_",fixed=TRUE))
  startyears = as.numeric(substr(fl2[,8],1,4))
  endyears = as.numeric(substr(fl2[,8],10,13))
  ...
  for(t in 1:length(years)){
    ...
    check = ifelse(years[t]>=startyears & years[t]<=endyears,1,0)
    fileidx = which(check==1)
    
    usefile = filesused[fileidx]
    ...
    dateidx = which(as.numeric(substr(filedates,1,4))==years[t])
    tmax = ncvar_get(nctest,nctest$var[[1]]$name,start=c(lonidx[1],latidx[1],dateidx[1]),count=c(length(lonidx),length(latidx),length(dateidx)))
    nc_close(nctest)
  ...
  # Create netcdf file
  if(var!="pr"){
    nc <- nc_create(paste("/home/woot0002/BCCA/",filelist2[1,1],"_",filelist2[1,2],"_",filelist2[1,3],"_",filelist2[1,4],"_",GCMs[i],"_histclimo.nc",sep="") ,  list(var1d,var2d) )
  } else {
    nc <- nc_create(paste("/home/woot0002/BCCA/",filelist2[1,1],"_",filelist2[1,2],"_",filelist2[1,3],"_",filelist2[1,4],"_",GCMs[i],"_histclimo.nc",sep="") ,  list(var1d,var2d,var3d,var4d,var5d) )
  }
