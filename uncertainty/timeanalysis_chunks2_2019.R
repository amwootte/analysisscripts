...
# South-Central Colorado = -107 -> -105, 37.5-> 40
...
lonidx4 = which(lon>= -107 & lon<= -105)
latidx4 = which(lat>= 37.5 & lat<= 40)
...
Vts4 = mean(V[lonidx4,latidx4],na.rm=TRUE) # SC CO time series
Mts4 = apply(M[lonidx4,latidx4,],3,mean,na.rm=TRUE)
Dts4 = apply(D[lonidx4,latidx4,],3,mean,na.rm=TRUE)
Sts4 = apply(S[lonidx4,latidx4,],3,mean,na.rm=TRUE)
Gts4 = apply(G[lonidx4,latidx4,],3,mean,na.rm=TRUE)
Tts4 = apply(T[lonidx4,latidx4,],3,mean,na.rm=TRUE)
...
FM4 = abs((1.65*sqrt(Mts4))/Gts4)
FD4 = abs((1.65*sqrt(Dts4))/Gts4)
FS4 = abs((1.65*sqrt(Sts4))/Gts4)
FV4 = abs((1.65*sqrt(Vts4))/Gts4)
FT4 = abs((1.65*sqrt(Tts4))/Gts4)
...
if(varname=="r1mm"){
  titlevar = "Average Number of Days with Rainfall"
  titlevarunits = "days"
}
...
#Frac = data.frame(year,Fvd,Fvm,Fm,Fd,Fs,Ft)
Frac = data.frame(year,FV4,FM4,FD4,FS4,FT4)

Frac2 = subset(Frac,year>2010)
Frac3 = Frac2

# normalized
plot(FT4/max(FT4,na.rm=TRUE)~year,data=Frac3,type="l",lwd=2,ylim=c(0,1),main=paste("Fractional Uncertainty ",titlevar,sep=""),ylab="Fractional Uncertainty",xlab="year")
lines(FV4/max(FT4,na.rm=TRUE)~year,data=Frac3,col="orange",lwd=2)
lines(FM4/max(FT4,na.rm=TRUE)~year,data=Frac3,col="darkblue",lwd=2)
lines(FD4/max(FT4,na.rm=TRUE)~year,data=Frac3,col="red",lwd=2)
lines(FS4/max(FT4,na.rm=TRUE)~year,data=Frac3,col="darkgreen",lwd=2)

#legend("topright",legend=c("Total","Int. Var GCM","Int. Var DS","GCM","DS","Scenario"),lwd=2,col=c("black","orange","brown","blue","red","green"))
legend("topleft",legend=c("Total","Nat. Variability","GCM","Downscaling","Scenario"),lwd=2,col=c("black","orange","darkblue","red","darkgreen"))

# non normalized
plot(FT4~year,data=Frac3,type="l",ylim=c(0,max(Frac3$FT,na.rm=TRUE)),lwd=2,main=paste("Fractional Uncertainty ",titlevar,sep=""),ylab="Fractional Uncertainty",xlab="year")
#lines(Fvm~year,data=Frac3,col="orange",lwd=2)
#lines(Fvd~year,data=Frac3,col="brown",lwd=2)
lines(FV4~year,data=Frac3,col="orange",lwd=2)
lines(FM4~year,data=Frac3,col="darkblue",lwd=2)
lines(FD4~year,data=Frac3,col="red",lwd=2)
lines(FS4~year,data=Frac3,col="darkgreen",lwd=2)
...
combodat2 = Frac = data.frame(year,Vts4,Mts4,Dts4,Sts4,Tts4)
datain2 = subset(combodat2,year>=2000)
datain2$leadtime = datain2$year-2000
...
plot(y.top~leadtime,col="orange",type="l",ylim=c(0,100),xlim=c(0,100),xlab="Leadtime in years from 2000",ylab="Fraction of Total Uncertainty (%)",main=paste("South-Central Colorado Fraction of Total Uncertainty\nfor ",titlevar,sep=""))
lines(y.mV~leadtime,col="darkgreen")
lines(y.mVS~leadtime,col="darkblue")
lines(y.mM~leadtime,col="red")
