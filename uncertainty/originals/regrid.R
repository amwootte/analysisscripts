...
files = c()
for(y in 1981:2012){
files = c(files,system(paste("ls /data/osu/prism/daily/combo/",y,"/*",sep=""),intern=TRUE))
}
...
# SERAP grid
newlat = c(33.5625, 33.6875, 33.8125, 33.9375, 34.0625, 34.1875, 34.3125, 34.4375, 34.5625, 34.6875, 34.8125, 34.9375, 35.0625, 35.1875, 35.3125, 35.4375, 35.5625, 35.6875, 35.8125, 35.9375, 36.0625, 36.1875, 36.3125, 36.4375, 36.5625, 36.6875)
...
# CLARENCE grid
#newlat = c(33.4875, 33.5741, 33.6605, 33.7469, 33.8332, 33.9194, 34.0056, 34.0916, 34.1776, 34.2634, 34.3492, 34.4349, 34.5205, 34.606, 34.6915, 34.7768, 34.862, 34.9472, 35.0323, 35.1173, 35.2022, 35.287, 35.3717, 35.4563, 35.5409, 35.6253, 35.7097, 35.794, 35.8782, 35.9623, 36.0463)

#newlon = c(279.959475, 280.0633125, 280.16715, 280.2709875, 280.374825, 280.4786625, 280.5825, 280.6863375, 280.790175, 280.8940125, 280.99785,    281.1016875, 281.205525, 281.3093625, 281.4132, 281.5170375, 281.620875,    281.7247125, 281.82855, 281.9323875, 282.036225, 282.1400625, 282.2439,    282.3477375, 282.451575, 282.5554125, 282.65925, 282.7630875, 282.866925,    282.9707625, 283.0746, 283.1784375, 283.282275, 283.3861125, 283.48995,    283.5937875, 283.697625, 283.8014625, 283.9053, 284.0091375)-360
...
newtmax = newtmin = newprcp = array(data=NA,dim=c(newrownum,newcolnum,length(files)))
...
tmaxname = test$var[[1]]$name
tminname = test$var[[2]]$name
prcpname = test$var[[4]]$name
...
tempunits = test$var[[1]]$units
prcpunits = test$var[[4]]$units
}

tmax = get.var.ncdf(test,tmaxname)
tmin = get.var.ncdf(test,tminname)
prcp = get.var.ncdf(test,"ppt")

close.ncdf(test)

lat2 = rev(lat)
tmax2 = tmax[,ncol(tmax):1]
tmin2 = tmin[,ncol(tmin):1]
prcp2 = prcp[,ncol(prcp):1]
#ptm = proc.time()

test2tmax = interp.surface.grid(list(x=lon,y=lat2,z=tmax2),grid.list=list(x=newlon,y=newlat))
newvar = matrix(test2tmax[[3]],nrow=length(newlon),ncol=length(newlat))
newtmax[,,f] = newvar

test2tmin = interp.surface.grid(list(x=lon,y=lat2,z=tmin2),grid.list=list(x=newlon,y=newlat))
newvar = matrix(test2tmin[[3]],nrow=length(newlon),ncol=length(newlat))
newtmin[,,f] = newvar

test2prcp = interp.surface.grid(list(x=lon,y=lat2,z=prcp2),grid.list=list(x=newlon,y=newlat))
newvar = matrix(test2prcp[[3]],nrow=length(newlon),ncol=length(newlat))
newprcp[,,f] = newvar
...
var1d <- var.def.ncdf(tmaxname, tempunits, list(dimX,dimY,dimT), mv )
var2d <- var.def.ncdf(tminname, tempunits, list(dimX,dimY,dimT), mv )
var3d <- var.def.ncdf(prcpname, prcpunits, list(dimX,dimY,dimT), mv )

# Create the test file
# Write some data to the file
# close ncdf

nc1 <- create.ncdf(paste("/projdata/dcerp/DSdata/CLAREnCE10/res12km/PRISM_12km_tmax.nc",sep="") , var1d )
put.var.ncdf( nc1, var1d, newtmax ) # no start or count: write all values\
close.ncdf(nc1)

nc2 <- create.ncdf("/projdata/dcerp/DSdata/CLAREnCE10/res12km/PRISM_12km_tmin.nc" , var2d )
put.var.ncdf( nc2, var2d, newtmin ) # no start or count: write all values\
close.ncdf(nc2)

nc3 <- create.ncdf("/projdata/dcerp/DSdata/CLAREnCE10/res12km/PRISM_12km_ppt.nc" , var3d )
put.var.ncdf( nc3, var3d, newprcp ) # no start or count: write all values\
close.ncdf(nc3)
