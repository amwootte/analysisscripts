...
fittedvals[[f]] = get.var.ncdf(test,"fittedvalues")
resvals[[f]] = get.var.ncdf(test,"residuals")
...
M_smooth = aperm(apply(M,c(1,2),filter,sides=2,filter=rep(1,10)/10),c(2,3,1))
D_smooth = aperm(apply(D,c(1,2),filter,sides=2,filter=rep(1,10)/10),c(2,3,1))
S_smooth = aperm(apply(S,c(1,2),filter,sides=2,filter=rep(1,10)/10),c(2,3,1))
...
G_smooth = aperm(apply(G,c(1,2),filter,sides=2,filter=rep(1,10)/10),c(2,3,1))
...
T_smooth = aperm(apply(T,c(1,2),filter,sides=2,filter=rep(1,10)/10),c(2,3,1))
...
fileout = paste("uncertainty/SE/",anfilepath,"/",varname,"_components_fix.nc",sep="")
