...
montharray[,,m] = apply(vardata[,,monidx],c(1,2),var,na.rm=TRUE)
} 
...
outputdata2 = lapply(outputdata,sqrt)
montharray2 = sqrt(montharray)

sddifference = lapply(outputdata2,"-",montharray2)

testsfc = list(x=lon,y=lat,z=sddifference[[1]][,,6])
...
outfilename = paste(varname,"_",GCMnamelist[GCMidx2,1],"_",datasetname,"_a2_varbias.nc",sep="")
fileout = paste("uncertainty/biascors/",outfilename,sep="")

var1d <- var.def.ncdf("mod_sd", dataunits1, list(dimX,dimY,dimT), mv )
var2d <- var.def.ncdf("obs_sd", dataunits1, list(dimX,dimY,dimT), mv )
var3d <- var.def.ncdf("diff_sd", dataunits2, list(dimX,dimY,dimT), mv )
...
put.var.ncdf( nc1, var2d, montharray2) # no start or count: write all values\
put.var.ncdf( nc1, var3d, sddifference[[i]]) # no start or count: write all values\
