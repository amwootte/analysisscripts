...
files = system(paste("ls /data4/data/OBS/",inname,"/",varname,"/",invar,"*.nc",sep=""),intern=TRUE)
  ...
  for(f in 1:length(files)){
    ...
    for(y in 1:length(years)){
      ...
      if(varname=="tasmax" | varname=="tasmin"){
        ...
        if(dim(vardata)[1]==length(lon)){
          tmp = apply(vardata[,,yearidx],c(1,2),mean,na.rm=TRUE)
          yeararray[,,y] = tmp
        } else {
        ...
        yearidx = which(as.numeric(substr(times,1,4))==years[y])
        if(varname=="tmax95") temp = ifelse((vardata[,,yearidx]-273.15)>=35,1,0)
        if(varname=="tmin32") temp = ifelse((vardata[,,yearidx]-273.15)<=0,1,0)
        if(varname=="pr25") temp = ifelse((vardata[,,yearidx]*86400)>=25.4,1,0)
        if(varname=="r1mm") temp = ifelse((vardata[,,yearidx]*86400)>=1,1,0)
        if(varname=="pr" | varname=="rx1day") temp = vardata[,,yearidx]*86400
        ...
        if(varname!="rx1day"){
          ...
          yeararray[,,y] = apply(temp,c(1,2),max,na.rm=TRUE)
          yeararray[,,y] = ifelse(is.na(vardata[,,1])==FALSE,yeararray[,,y],NA)
        }
      }
