#################################
#
# Downscaled Data Re-gridding Script - MACA
#
# Adrienne Wootten 6/18/2015
#
#################################
#
# Required Arguments
...
files = system(paste("ls /projdata/dcerp/DSdata/loca/SE/",var,"*",period,"*.nc",sep=""),intern=TRUE)
...
lon = ncvar_get(test,lonname)-360
lat = ncvar_get(test,latname)

testvar = ncvar_get(test,modelname)
nc_close(test)
...
test2 = interp.surface.grid(list(x=lon2,y=lat2,z=testvar[,,j]),grid.list=list(x=newlon,y=newlat))
...
fileout = strsplit(files[f],"/",fixed=TRUE)[[1]][7]

nc <- create.ncdf(paste("loca/SE/res15km/",fileout,sep="") , var1d )
