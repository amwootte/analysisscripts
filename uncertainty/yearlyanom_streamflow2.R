...
load(paste("/home/woot0002/CPREP_",datasetname,"_2006-2099_fixed_climovars.Rdata",sep=""))
...
for(f in 1:length(projresults)){
  ...
  if(f==1){
    varnames = names(histresults[[1]])
    varidxhist = which(varnames==varname) 
    varnames = names(projresults[[1]])
    varidxproj = which(varnames==varname) 
  }
  
  histtmp = histresults[[histidx]][,varidxhist]
  projtmp = projresults[[f]][,varidxproj]
  ...
  if(varname == "TMAX" | varname=="TMIN" | varname == "MAXPRCP1W_2W" | varname == "MAXPRCP1W_12M" | varname == "MAXTMAX1W_2W" | varname == "TMAXAVG_1W" | varname == "TAVG_3M" | varname == "MAXTMAX1W_12M" | varname == "TAVG_12M"){
  ...
  if(varname == "PRCP" | varname == "PRCPAVG_3D" | varname == "MDRN_4W" | varname == "HOT90_3M" | varname == "HOT90_12M" | varname == "PRCPAVG_6M" | varname == "PRCPAVG_12M"){
    tmpyearly = aggregate(tmpdataframe[,2],by=list(year=tmpdataframe$year),sum,na.rm=TRUE)
  }
