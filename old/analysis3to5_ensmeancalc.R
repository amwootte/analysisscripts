###############
# 3^5 Projected Change Analyses - ensemble means
...
varname = "frd" # short name for the variable of interest options include tasmax, tasmin, pr, tmax95, tmax100, tmin32, tmin28, pr25, and pr50
...
futureperiod = c(2041,2070) # future period for difference calcs
# recommended to be 30 years. Format above should be c(startyear,endyear)
...
appfunc = "lastfreeze" # which functions do you apply for yearly calculations? "mean" is used for getting annual average temps for instance. "sum" would be used for annual total rainfall and thresholds
...
for(i in 1:length(projfilelist)){
  ...
  yearlyoutput = netcdftoyearlycalcs(projfilelist[i],varname=varin,dimnames=c("lon","lat","time"),threscalc=TC,thres=TH,condition=cond,yearlydataperiod=c(futureperiod[1],futureperiod[2]),datesdataperiod=datesin,appliedfunction=appfunc)
  
  if(i==1){
    projlist = array(NA,dim=c(length(lon),length(lat),length(projfilelist)))
  }
  
  projlist[,,i] = climocalc(yearlyoutput[[3]],yearlydataperiod=c(futureperiod[1],futureperiod[2]),climoperiod=c(futureperiod[1],futureperiod[2]))
  rm(yearlyoutput)
  gc()
  ptmend = proc.time()
...
projsg1 = array(NA,dim=c(length(lon),length(lat),length(unique(projfilebreakdown$scen))))
...
var7d <- ncvar_def("projmeandiif_rcp85",diffunits,longname="Mean Projected Change RCP 8.5", list(dimX,dimY), mv )
