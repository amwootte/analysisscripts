...
projmean = ncvar_get(test,"projmean")
histmean = ncvar_get(test,"histmean")
...
aggregate(projfilebreakdown[,5:6],by=list(scen=projfilebreakdown$scen),mean,na.rm=TRUE)

aggregate(projfilebreakdown[,5:6],by=list(scenGCM=paste(projfilebreakdown$scen,projfilebreakdown$GCM,sep="_")),mean,na.rm=TRUE)
