####
# Spring Phenology R functions v. 0.2
# AMW 6/24/2019
#
...
# - No changes to notes from version 0.1.
#
...
calc_si = function(TMAX,TMIN,lat){
  ...
  #           There is one value of FSEI each for the early and late false springs. [early false spring FSEI, late false spring FSEI]
  #
  ...
  for(i in 1:ncol(TMAX)){
    ...
    dlen = daylength(daystop,yearlength=length(DOY),lat=lat) # Calculating daylength
...
####
# Solar declination - SI-x supporting function, no longer used. From R's solrad package.
declination = function(DOY,yearlength=365){
  # INPUTS
  # DOY - scalar day of year, from 1 to 366 or 1 to 365 depending on the calendar of the date
  # yearlength - scalar total number of days for the given year of calculation
  #
  # OUTPUTS
  # Delta - solar declination
  
  Delta=23.45*sin(pi/180*360/yearlength*(284+DOY))
  Delta
}
...
daylength = function(daystop,yearlength,lat){
  # INPUTS
  # daystop - scalar day of year at which to end calculations
  # yearlength - scalar total number of days for the given year of calculation, passed to declination
...
falsesprings = function(SI=c(leafidx,bloomidx),freezedata){
  # INPUTS
  ...
  #       EFS occurs when a hard freeze happens 7 or more days after FLI
  ...
  EFSidx = which(freezedata$DOY>=(SI[1]+7))
