...
var = "pr"
OBSfile = paste("/home/woot0002/RCMES/data/OBS/",var,"_DAYMETv4_mon_CONUS_1980-2014.nc",sep="")

filelist = system(paste("ls /data4/data/CMIP6/monthlyclimo/",var,"*.nc",sep=""),intern=TRUE)
GCMfiles = filelist[grep("ssp585",filelist)]

climotype = "mon"

mask = TRUE
maskregion = "SGP-NCA"
maskfile = "/home/woot0002/RCMES/masks/SGP-NCA_DAYMETv4-1km_mask.nc"
...
histdates = seq(as.Date("2070-01-15"),as.Date("2099-12-15"),by="month")
years = 2070:2099
...
for(f in 1:length(GCMfiles)){
  ...
  #checkdat = apply(outputdata[[f]],3,mean,na.rm=TRUE)
  ...
  if(climotype=="JAN" | climotype=="FEB" | climotype=="MAR" | climotype=="APR"| climotype=="MAY" | climotype=="JUN" | climotype=="JUL" | climotype=="AUG" | climotype=="SEP" | climotype=="OCT" | climotype=="NOV" | climotype=="DEC"){
    outtmp = array(NA,dim=c(length(lon),length(lat),length(years)))
    if(climotype=="JAN") m=1
    if(climotype=="FEB") m=2
    if(climotype=="MAR") m=3
    if(climotype=="APR") m=4
...
#####
# write netcdf - annual and single month
...
fileout = paste("/home/woot0002/RCMES/",var,"_EAA_climo_ssp585_",maskregion,"_",climotype,".nc",sep="")
...
if(climotype=="monthly"){
  ...
  fileout = paste("/home/woot0002/RCMES/",var,"_EAA_climo_ssp585_",maskregion,"_monthly.nc",sep="")
