...
histfilelist = system(paste("ls /data4/data/DS_proj/LOCA/",varin,"/historical/*.nc",sep=""),intern=TRUE)
projfilelist = system(paste("ls /data4/data/DS_proj/LOCA/",varin,"/future/*.nc",sep=""),intern=TRUE)
...
filebreakdown2[,2] = as.numeric(filebreakdown2[,2])
filebreakdown2[,3] = as.numeric(substr(filebreakdown2[,3],1,4))
filebreakdown3 = data.frame(filebreakdown[,3:4],filebreakdown2)
names(filebreakdown3) = c("GCM","experiment","scen","startyear","endyear")
startyears = as.numeric(substr(filebreakdown[,5],7,10))
filebreakdown3$startyear = startyears
projfilebreakdown = filebreakdown3
rm(filebreakdown3)

filebreakdown = do.call(rbind,strsplit(histfilelist,"_",fixed=TRUE))
filebreakdown3 = data.frame(filebreakdown[,3:4])
names(filebreakdown3) = c("GCM","experiment")
...
projfilebreakdown$GCMscen = paste(projfilebreakdown$GCM,projfilebreakdown$scen,sep="_")
GCMscens = as.character(unique(projfilebreakdown$GCMscen))
projfilebreakdown$startyear = as.numeric(projfilebreakdown$startyear)
projfilebreakdown$endyear = as.numeric(projfilebreakdown$endyear)


for(i in 1:length(GCMscens)){
  
  filesin = which(projfilebreakdown$GCMscen == GCMscens[i] & as.numeric(projfilebreakdown$startyear)>=years[1])
  
  ptm = proc.time()
  message("Starting work on file ",GCMscens[i])
  ...
  for(j in 1:length(filesin)){
    ...
    if(varunits=="kg m-2 s-1"){
      tmp = c(tmp,apply(tempdata,3,mean,na.rm=TRUE)*86400)
    } else {
  ...
  if(i==1) yearframe = array(NA,dim=c(365,ncol=length(years),length(GCMscens)))
  ...
  gc()
  ptmend = proc.time()
  message("Finished with file ",i," / ",length(GCMscens))
  message("Time to complete gathering: ",ptmend[3]-ptm[3]," secs")
}
...
for(f in 1:length(histfilelist)){
plot(histdailycumanom[,f],type="l",main = paste(histfilebreakdown$GCM[f],"LOCA",sep=" "))
}
...
save(list=c("GCMscens","histdailycumanom","histyearframe","histfilebreakdown","projyearframe","projfilebreakdown","projdailycumanom_end","histyears","endyears","projyears"),file=paste("wetseasontesting_LOCA_",loc_name,".Rdata",sep=""))
