...
files = system("ls /home/woot0002/obs/*.nc",intern=TRUE)
...
for(f in 1:length(files)){
  ...
  missingval = test$var[[1]]$missval
    ...
    varname3 = "tmaxclimo"
    varname4 = "tmax95climo"
    units3 = "degrees_C"
    units4 = "days"
  
    varname5 = "tminclimo"
    varname6 = "tmin32climo"
    units5 = "degrees_C"
    units6 = "days"
  ...
  testvar6 = ncvar_get(test,varname6)
  nc_close(test)
  
  #lat2= lat
  #lon2 =lon
  
  newmatrix1 = newmatrix2 = newmatrix3 = newmatrix4 = newmatrix5 = newmatrix6 = array(data=NA,dim=c(newrownum,newcolnum,timelength))
  
  for(j in 1:timelength){
    
    ptm = proc.time()
    
    int1 = interp.surface.grid(list(x=lon,y=lat,z=testvar1[,,j]),grid.list=list(x=newlon2,y=newlat))
    int2 = interp.surface.grid(list(x=lon,y=lat,z=testvar2[,,j]),grid.list=list(x=newlon2,y=newlat))
    int3 = interp.surface.grid(list(x=lon,y=lat,z=testvar3[,,j]),grid.list=list(x=newlon2,y=newlat))
    int4 = interp.surface.grid(list(x=lon,y=lat,z=testvar4[,,j]),grid.list=list(x=newlon2,y=newlat))
    int5 = interp.surface.grid(list(x=lon,y=lat,z=testvar5[,,j]),grid.list=list(x=newlon2,y=newlat))
    int6 = interp.surface.grid(list(x=lon,y=lat,z=testvar6[,,j]),grid.list=list(x=newlon2,y=newlat))
    
    #pr
    newvar = matrix(int1[[3]],nrow=length(newlon2),ncol=length(newlat))
    newvar = ifelse(newvar<0,0,newvar)
    newmatrix1[,,j] = newvar
    
    #pr50
    newvar = matrix(int2[[3]],nrow=length(newlon2),ncol=length(newlat))
    newvar = ifelse(newvar<0,0,newvar)
    newmatrix2[,,j] = newvar
    
    #tmax
    newvar = matrix(int3[[3]],nrow=length(newlon2),ncol=length(newlat))
    newmatrix3[,,j] = newvar
    
    #tmax95
    newvar = matrix(int4[[3]],nrow=length(newlon2),ncol=length(newlat))
    newvar = ifelse(newvar<0,0,newvar)
    newmatrix4[,,j] = newvar
    
    #tmin
    newvar = matrix(int5[[3]],nrow=length(newlon2),ncol=length(newlat))
    newmatrix5[,,j] = newvar
    
    #tmin32
    newvar = matrix(int6[[3]],nrow=length(newlon2),ncol=length(newlat))
    newvar = ifelse(newvar<0,0,newvar)
    newmatrix6[,,j] = newvar
  ...
  var6d <- ncvar_def(varname6,units6, list(dimX,dimY,dimT), mv )
  
  # Create the test file
  fileout = strsplit(files[f],"/",fixed=TRUE)[[1]][5]
  
  nc <- nc_create(paste("/home/woot0002/obs/regrid/",fileout,sep="") ,  list(var1d,var2d,var3d,var4d,var5d,var6d) )
  
  # Write some data to the file
  ncvar_put(nc, var1d, newmatrix1) # no start or count: write all values\
  ncvar_put(nc, var2d, newmatrix2)
  ncvar_put(nc, var3d, newmatrix3)
  ncvar_put(nc, var4d, newmatrix4) # no start or count: write all values\
  ncvar_put(nc, var5d, newmatrix5)
  ncvar_put(nc, var6d, newmatrix6)
