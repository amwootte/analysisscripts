...
args = c("maca_metdata","pr")

dataset = args[1]
var = args[2]

files = system(paste("ls /home/woot0002/MACA_METDATA/*",var,"*.nc",sep=""),intern=TRUE)
...
for(f in 1:length(files)){
  ...
  testsfc = list(x=lon,y=lat,z=testvar5[,,1])
  testsfc2 = list(x=newlon,y=newlat,z=newmatrix5[,,1])
  ...
  if(var!="pr"){
  nc <- nc_create(paste("/home/woot0002/MACA_METDATA/regrid/",fileout,sep="") ,  list(var1d,var4d) )
  ...
  ncvar_put(nc, var4d, newmatrix2) # no start or count: write all values\
    ...
    nc <- nc_create(paste("/home/woot0002/MACA_METDATA/regrid/",fileout,sep="") ,  list(var1d,var2d,var3d,var4d,var5d) )
