...
#usevalues = which(is.na(fulldata2$streamflow_mean)==FALSE & is.na(fulldata2$PRCPAVG_3D)==FALSE)
#ccf(fulldata2$PRCPAVG_3D[usevalues],fulldata2$streamflow_mean[usevalues],type="correlation")
...
usevaridx = c(18,43,45,69,118,93,159,37,77,53,54,78,163,42)
...
namesused = names(traingroup)[usevaridx]
...
allRMSE2 = sqrt(mean((predictions2-ymat2)^2))
allcor2 = cor(ymat2,predictions2)
...
lines(density(logpred),col="blue")#logpred[-NAidx2]
legend("topright",legend=c("Observed","Modeled"),col=c("black","blue"),lwd=1)

logpred = log(predictions2)
NAidx2=which(is.na(logpred)==TRUE | logpred == -Inf)

plot(density(log(ymat2)),xlab="ln(streamflow)",main="Prediction period streamflow pdfs")
lines(density(logpred[-NAidx2]),col="blue")
legend("topleft",legend=c("Observed","Modeled"),col=c("black","blue"),lwd=1)
...
for(c in 0:(length(allnames)-1)){
  ...
  corswithin = list()
  RMSEswithin = list()
  pdf(paste("/home/woot0002/streamflowregressiontests_3daymovingmean_",c,"varsout_",streamflowID,".pdf",sep=""),onefile=TRUE,width=5,height=5)
...
save(list=c("varsusedall","corsusedall","RMSEsusedall"),file=paste("/home/woot0002/streamflowtests_3daymovingmean_",streamflowID,".Rdata",sep=""))
