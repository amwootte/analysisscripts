...
save(list="meansdat_tmax",file=paste("WeightedMeansVars_tmax_",var,"_WU",weightingused,"_SA",stateapplied,"_wBMA.Rdata",sep=""))
...
save(list=c("GCMunweightedmean_bias_pr","GCMunweightedmean_bias_tmax",
            ...
            "LOCASIcmean_bias_pr","LOCASIcmean_bias_tmax",
            "GCMBMAmean_bias_pr","GCMBMAmean_bias_tmax",
            "LOCABMAmean_bias_pr","LOCABMAmean_bias_tmax"),file=paste("Biasesbasedon_",var,"_WU",weightingused,"_SA",stateapplied,"_weighting_wBMA.Rdata",sep=""))
...
save(list=c("GCMunweightedmean_change_pr","GCMunweightedmean_change_tmax",
            ...
            "LOCASIcmean_change_pr","LOCASIcmean_change_tmax",
            "GCMBMAmean_change_pr","GCMBMAmean_change_tmax",
            "LOCABMAmean_change_pr","LOCABMAmean_change_tmax"),file=paste("Changesbasedon_",var,"_WU",weightingused,"_SA",stateapplied,"_weighting_wBMA.Rdata",sep=""))
...
save(list=c("GCMunweightedvar_change_pr","GCMunweightedvar_change_tmax",
            ...
            "LOCASIcvar_change_pr","LOCASIcvar_change_tmax",
            "GCMBMAvar_change_pr","GCMBMAvar_change_tmax",
            "LOCABMAvar_change_pr","LOCABMAvar_change_tmax"),file=paste("ChangeVarsbasedon_",var,"_WU",weightingused,"_SA",stateapplied,"_weighting_wBMA.Rdata",sep=""))
...
meansdatBMA_pr = meansdatBMA_tmax = NULL
for(i in 1:100){ # meansdatBMA_pr loop
  
  if(stateapplied!="full"){
    GCMBMAhm = GCMBMAhistmean_pr[,,i]
    GCMBMAcm = GCMBMAchangemean_pr[,,i]
    GCMBMAcv = GCMBMAchangevar_pr[,,i]
    
    GCMBMAhm = ifelse(regionmask==1,GCMBMAhm,NA)
    GCMBMAcm = ifelse(regionmask==1,GCMBMAcm,NA)
    GCMBMAcv = ifelse(regionmask==1,GCMBMAcv,NA)
    
    LOCABMAhm = LOCABMAhistmean_pr[,,i]
    LOCABMAcm = LOCABMAchangemean_pr[,,i]
    LOCABMAcv = LOCABMAchangevar_pr[,,i]
    
    LOCABMAhm = ifelse(regionmask==1,LOCABMAhm,NA)
    LOCABMAcm = ifelse(regionmask==1,LOCABMAcm,NA)
    LOCABMAcv = ifelse(regionmask==1,LOCABMAcv,NA)
    ...
    region = stateapplied
      ...
      RMSE_GCM = sqrt(mean((GCMBMAhm-OBS_pr)^2,na.rm=TRUE))
      RMSE_LOCA = sqrt(mean((LOCABMAhm-OBS_pr)^2,na.rm=TRUE))
    ...
    meansdatBMA_pr = rbind(meansdatBMA_pr,meansframe)
  }

meansdatBMA_pr$changesd = sqrt(meansdatBMA_pr$changevars)
meansdatBMA_pr$bias = meansdatBMA_pr$histmeans-meansdatBMA_pr$obs
meansdatBMA_pr$region <- stateapplied
meansdatBMA_pr$changesd = sqrt(meansdatBMA_pr$changevars)

save(list="meansdatBMA_pr",file=paste("BMAPost_WeightedMeansVars_pr_",var,"_WU",weightingused,"_SA",stateapplied,".Rdata",sep=""))

###

for(i in 1:100){ # meansdatBMA_tmax loop
  
  if(stateapplied!="full"){
    GCMBMAhm = GCMBMAhistmean_tmax[,,i]
    GCMBMAcm = GCMBMAchangemean_tmax[,,i]
    GCMBMAcv = GCMBMAchangevar_tmax[,,i]
    
    GCMBMAhm = ifelse(regionmask==1,GCMBMAhm,NA)
    GCMBMAcm = ifelse(regionmask==1,GCMBMAcm,NA)
    GCMBMAcv = ifelse(regionmask==1,GCMBMAcv,NA)
    
    LOCABMAhm = LOCABMAhistmean_tmax[,,i]
    LOCABMAcm = LOCABMAchangemean_tmax[,,i]
    LOCABMAcv = LOCABMAchangevar_tmax[,,i]
    ...
    RMSE_GCM = sqrt(mean((GCMBMAhm-OBS_tmax)^2,na.rm=TRUE))
    RMSE_LOCA = sqrt(mean((LOCABMAhm-OBS_tmax)^2,na.rm=TRUE))
  ...
  meansframe = data.frame(BMAmem,region,DS,histmeans,obs,changemeans,changevars,rmse)
  meansdatBMA_tmax = rbind(meansdatBMA_tmax,meansframe)
}

meansdatBMA_tmax$changesd = sqrt(meansdatBMA_tmax$changevars)
meansdatBMA_tmax$bias = meansdatBMA_tmax$histmeans-meansdatBMA_tmax$obs
meansdatBMA_tmax$region <- stateapplied
meansdatBMA_tmax$changesd = sqrt(meansdatBMA_tmax$changevars)

save(list="meansdatBMA_tmax",file=paste("BMAPost_WeightedMeansVars_tmax_",var,"_WU",weightingused,"_SA",stateapplied,".Rdata",sep=""))
}
