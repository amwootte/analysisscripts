...
datasetname = "Maurer" # common name of dataset to start with
...
if(varname == "pr25" | varname == "pr50" | varname == "r1mm" | varname == "rx1day" | varname == "rx5day" | varname == "cwd" | varname == "cdd"){ varin="pr"; varin="pr";}
...
for(y in 1:length(years)){
    test = nc_open(paste("/data4/data/OBS/Maurer/",varin2,"/nldas_met_update.obs.daily.",varin2,".",years[y],".nc",sep=""))
    ...
    vardata = ncvar_get(test,test$var[[3]]$name)
    
    time = ncvar_get(test,"time")
    timeunits = test$var[[3]]$dim[[3]]$units
    nc_close(test)
    ...
    for(m in 1:12){
      ...
      if(varname == "tasmax" | varname == "tasmin"){
        temp2 = apply(vardata[,,monidx],c(1,2),mean,na.rm=TRUE)
      }
      ...
      if(varname == "tmax95" | varname == "tmax100" | varname == "tmin32" | varname == "tmin28" | varname == "pr25" | varname == "pr50" | varname == "r1mm"){
        ...
        if(varname == "tmax95")  temp = ifelse(vardata[,,monidx]>=35,1,0)
        if(varname == "tmin32")  temp = ifelse(vardata[,,monidx]<=0,1,0)
...
fileout = paste("/home/woot0002/monthlyclimo/",varname,"_maurer_histclimo_mon.nc",sep="")
...
send.mail(from = "amwootte@ou.edu",
          ...
          body = paste("Maurer climatology calculations complete for var: ",varname," \n ",Sys.time(),sep=""), 
          authenticate = TRUE,
