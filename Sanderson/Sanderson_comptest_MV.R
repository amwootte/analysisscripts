...
for(s in 1:5){
  if(s<=4){
    ...
    histmeans_GCM_pr = c(mean(GCMunweightedmean_hist_pr[regionidx],na.rm=TRUE),mean(GCMskillmean_hist_pr[regionidx],na.rm=TRUE),mean(GCMSIhmean_hist_pr[regionidx],na.rm=TRUE),mean(GCMSIcmean_hist_pr[regionidx],na.rm=TRUE)) 
    histmeans_LOCA_pr = c(mean(LOCAunweightedmean_hist_pr[regionidx],na.rm=TRUE),mean(LOCAskillmean_hist_pr[regionidx],na.rm=TRUE),mean(LOCASIhmean_hist_pr[regionidx],na.rm=TRUE),mean(LOCASIcmean_hist_pr[regionidx],na.rm=TRUE))
    
    changemeans_GCM_pr = c(mean(GCMunweightedmean_change_pr[regionidx],na.rm=TRUE),mean(GCMskillmean_change_pr[regionidx],na.rm=TRUE),mean(GCMSIhmean_change_pr[regionidx],na.rm=TRUE),mean(GCMSIcmean_change_pr[regionidx],na.rm=TRUE)) 
    changemeans_LOCA_pr = c(mean(LOCAunweightedmean_change_pr[regionidx],na.rm=TRUE),mean(LOCAskillmean_change_pr[regionidx],na.rm=TRUE),mean(LOCASIhmean_change_pr[regionidx],na.rm=TRUE),mean(LOCASIcmean_change_pr[regionidx],na.rm=TRUE)) 
    
    changevars_GCM_pr = c(mean(GCMunweightedvar_change_pr[regionidx],na.rm=TRUE),mean(GCMskillvar_change_pr[regionidx],na.rm=TRUE),mean(GCMSIhvar_change_pr[regionidx],na.rm=TRUE),mean(GCMSIcvar_change_pr[regionidx],na.rm=TRUE)) 
    changevars_LOCA_pr = c(mean(LOCAunweightedvar_change_pr[regionidx],na.rm=TRUE),mean(LOCAskillvar_change_pr[regionidx],na.rm=TRUE),mean(LOCASIhvar_change_pr[regionidx],na.rm=TRUE),mean(LOCASIcvar_change_pr[regionidx],na.rm=TRUE)) 
    
    obs = rep(mean(OBS_pr[regionidx],na.rm=TRUE),8)
    region = rep(statelist[s],8)
    
    RMSE_GCM_pr = c(sqrt(mean((GCMunweightedmean_bias_pr[regionidx])^2,na.rm=TRUE)),
                 sqrt(mean((GCMskillmean_bias_pr[regionidx])^2,na.rm=TRUE)),
                 sqrt(mean((GCMSIhmean_bias_pr[regionidx])^2,na.rm=TRUE)),
                 sqrt(mean((GCMSIcmean_bias_pr[regionidx])^2,na.rm=TRUE)))
    
    RMSE_LOCA_pr = c(sqrt(mean((LOCAunweightedmean_bias_pr[regionidx])^2,na.rm=TRUE)),
                  sqrt(mean((LOCAskillmean_bias_pr[regionidx])^2,na.rm=TRUE)),
                  sqrt(mean((LOCASIhmean_bias_pr[regionidx])^2,na.rm=TRUE)),
                  sqrt(mean((LOCASIcmean_bias_pr[regionidx])^2,na.rm=TRUE)))
    ...
    obs = rep(mean(OBS_pr,na.rm=TRUE),8)
    region = rep("full",8)
...
save(list="meansdat_pr",file=paste("WeightedMeansVars_pr_",var,".Rdata",sep=""))
...
for(s in 1:5){
  if(s<=4){
    regionidx = which(regionmasklist[[s]]==1)
    histmeans_GCM_tmax = c(mean(GCMunweightedmean_hist_tmax[regionidx],na.rm=TRUE),mean(GCMskillmean_hist_tmax[regionidx],na.rm=TRUE),mean(GCMSIhmean_hist_tmax[regionidx],na.rm=TRUE),mean(GCMSIcmean_hist_tmax[regionidx],na.rm=TRUE)) 
    histmeans_LOCA_tmax = c(mean(LOCAunweightedmean_hist_tmax[regionidx],na.rm=TRUE),mean(LOCAskillmean_hist_tmax[regionidx],na.rm=TRUE),mean(LOCASIhmean_hist_tmax[regionidx],na.rm=TRUE),mean(LOCASIcmean_hist_tmax[regionidx],na.rm=TRUE))
    
    changemeans_GCM_tmax = c(mean(GCMunweightedmean_change_tmax[regionidx],na.rm=TRUE),mean(GCMskillmean_change_tmax[regionidx],na.rm=TRUE),mean(GCMSIhmean_change_tmax[regionidx],na.rm=TRUE),mean(GCMSIcmean_change_tmax[regionidx],na.rm=TRUE)) 
    changemeans_LOCA_tmax = c(mean(LOCAunweightedmean_change_tmax[regionidx],na.rm=TRUE),mean(LOCAskillmean_change_tmax[regionidx],na.rm=TRUE),mean(LOCASIhmean_change_tmax[regionidx],na.rm=TRUE),mean(LOCASIcmean_change_tmax[regionidx],na.rm=TRUE)) 
    
    changevars_GCM_tmax = c(mean(GCMunweightedvar_change_tmax[regionidx],na.rm=TRUE),mean(GCMskillvar_change_tmax[regionidx],na.rm=TRUE),mean(GCMSIhvar_change_tmax[regionidx],na.rm=TRUE),mean(GCMSIcvar_change_tmax[regionidx],na.rm=TRUE)) 
    changevars_LOCA_tmax = c(mean(LOCAunweightedvar_change_tmax[regionidx],na.rm=TRUE),mean(LOCAskillvar_change_tmax[regionidx],na.rm=TRUE),mean(LOCASIhvar_change_tmax[regionidx],na.rm=TRUE),mean(LOCASIcvar_change_tmax[regionidx],na.rm=TRUE)) 
    
    obs = rep(mean(OBS_tmax[regionidx],na.rm=TRUE),8)
    region = rep(statelist[s],8)
    
    RMSE_GCM_tmax = c(sqrt(mean((GCMunweightedmean_bias_tmax[regionidx])^2,na.rm=TRUE)),
                    sqrt(mean((GCMskillmean_bias_tmax[regionidx])^2,na.rm=TRUE)),
                    sqrt(mean((GCMSIhmean_bias_tmax[regionidx])^2,na.rm=TRUE)),
                    sqrt(mean((GCMSIcmean_bias_tmax[regionidx])^2,na.rm=TRUE)))
    
    RMSE_LOCA_tmax = c(sqrt(mean((LOCAunweightedmean_bias_tmax[regionidx])^2,na.rm=TRUE)),
                     sqrt(mean((LOCAskillmean_bias_tmax[regionidx])^2,na.rm=TRUE)),
                     sqrt(mean((LOCASIhmean_bias_tmax[regionidx])^2,na.rm=TRUE)),
                     sqrt(mean((LOCASIcmean_bias_tmax[regionidx])^2,na.rm=TRUE)))
    ...
    obs = rep(mean(OBS_tmax,na.rm=TRUE),8)
    region = rep("full",8)
...
save(list="meansdat_tmax",file=paste("WeightedMeansVars_tmax_",var,".Rdata",sep=""))
...
save(list=c("GCMunweightedmean_bias_pr","GCMunweightedmean_bias_tmax",
            ...
            "LOCASIcmean_bias_pr","LOCASIcmean_bias_tmax"),file=paste("Biasesbasedon_",var,"_weighting.Rdata",sep=""))
...
save(list=c("GCMunweightedmean_change_pr","GCMunweightedmean_change_tmax",
            ...
            "LOCASIcmean_change_pr","LOCASIcmean_change_tmax"),file=paste("Changesbasedon_",var,"_weighting.Rdata",sep=""))
...
save(list=c("GCMunweightedvar_change_pr","GCMunweightedvar_change_tmax",
            ...
            "LOCASIcvar_change_pr","LOCASIcvar_change_tmax"),file=paste("ChangeVarsbasedon_",var,"_weighting.Rdata",sep=""))
