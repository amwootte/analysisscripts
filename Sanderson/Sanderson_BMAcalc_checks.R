...
if(weightingused=="full"){
  ...
  BMAweightsLOCA = read.table("posterior_BMA_combo_LOCA_",var,".txt")
  BMAweights_GCM = read.table("best_BMA_combo_",var,".txt")
  BMAweights_LOCA = read.table("best_BMA_combo_LOCA_",var,".txt")
} else {
...
for(i in 1:26){
  ...
  GCMBMAhistmean_pr[,,(b-11)]=GCMBMAmean_hist
  LOCABMAhistmean_pr[,,(b-11)]=LOCABMAmean_hist
}
...
for(b in 12:111){
  ...
  GCMBMAhistmean_tmax[,,(b-11)]=GCMBMAmean_hist
  LOCABMAhistmean_tmax[,,(b-11)]=LOCABMAmean_hist
}

GCMtmaxbias = array(NA,dim=dim(GCMBMAhistmean_tmax))
LOCAtmaxbias = array(NA,dim=dim(GCMBMAhistmean_tmax))

for(i in 1:100){
  GCMtmaxbias[,,i] = GCMBMAhistmean_tmax[,,i]-GCMhvardatalist_tmax[[27]]
  LOCAtmaxbias[,,i] = LOCABMAhistmean_tmax[,,i]-GCMhvardatalist_tmax[[27]]
}

GCMtmaxRMSE = sqrt(apply(GCMtmaxbias^2,3,mean,na.rm=TRUE))
LOCAtmaxRMSE = sqrt(apply(LOCAtmaxbias^2,3,mean,na.rm=TRUE))

GCMprRMSE = sqrt(mean(GCMprbias^2,na.rm=TRUE))
LOCAprRMSE = sqrt(mean(LOCAprbias^2,na.rm=TRUE))
