...
W = WA = array(NA,dim=c(nummodels,length(Dqvec)))

for(j in 1:length(Dqvec)){
  W[,j] = Wqmat[,j]*Wc*Wu
  WA[,j] = W[,j]/sum(W[,j])
}
...
weightedmeanarray = array(0,dim=c(length(lon),length(lat),12,length(Dqvec)))

for(m in 1:12){
  for(c in 1:length(Dqvec)){
    for(i in 1:nummodels){
...
for(i in 1:length(obsidx)){
  ...
  for(c in 1:length(Dqvec)){
    ...
    RMSEval = RMSE(exp=moddat,obs=obsdat)
    frametmp = data.frame(obs,Dq,RMSEval)
...
for(i in 1:5){
  ...
  if(i==1){
    plot(RMSErel~Dq,data=tmpframe,type="l",lwd=2,col="black",ylim=range(tasmaxweightedRMSE$RMSErel),main="Tasmax RMSE scores",ylab="RMSE score of weighted mean relative to unweighted mean")
  } else {
    lines(RMSErel~Dq,data=tmpframe,col=cols[i],lwd=2)
  }
...
for(i in 1:5){
  ...
  if(i==1){
    plot(RMSErel~Dq,data=tmpframe,type="l",lwd=2,col="black",main="Tasmin RMSE scores",ylim=range(tasminweightedRMSE$RMSErel),ylab="RMSE score of weighted mean relative to unweighted mean")
  } else {
    lines(RMSErel~Dq,data=tmpframe,col=cols[i],lwd=2)
  }
...
for(i in 1:5){
  ...
  if(i==1){
    plot(RMSErel~Dq,data=tmpframe,type="l",lwd=2,col="black",main="Pr RMSE scores",ylim=range(prweightedRMSE$RMSErel),ylab="RMSE score of weighted mean relative to unweighted mean")
  } else {
    lines(RMSErel~Dq,data=tmpframe,col=cols[i],lwd=2)
  }
...
for(c in 1:length(Dqvec)){
  ...
  tmp = subset(tasmaxweightedRMSE,Dq==Dqvec[c])
  tmaxtmp = apply(tmp[,3:4],2,mean,na.rm=TRUE)
  RMSEval = tmaxtmp[1]
  RMSErel = tmaxtmp[2]
  tmpframe = data.frame(Dq,RMSEval,RMSErel)
  tasmaxRMSEagg = rbind(tasmaxRMSEagg,tmpframe)
  
  tmp = subset(tasminweightedRMSE,Dq==Dqvec[c])
  tmaxtmp = apply(tmp[,3:4],2,mean,na.rm=TRUE)
  RMSEval = tmaxtmp[1]
  RMSErel = tmaxtmp[2]
  tmpframe = data.frame(Dq,RMSEval,RMSErel)
  ...
  tmp = subset(prweightedRMSE,Dq==Dqvec[c])
  tmaxtmp = apply(tmp[,3:4],2,mean,na.rm=TRUE)
  RMSEval = tmaxtmp[1]
  RMSErel = tmaxtmp[2]
  tmpframe = data.frame(Dq,RMSEval,RMSErel)
...
RMSEagg2 = aggregate(RMSEagg$RMSErel,by=list(Dq=RMSEagg$Dq),mean,na.rm=TRUE)

plot(x~Dq,data=RMSEagg2,type="l",lwd=2,col="black",main="All RMSE scores by Dq radius",ylim=range(c(prRMSEagg$RMSErel,tasmaxRMSEagg$RMSErel,tasminRMSEagg$RMSErel)),ylab="RMSE score of weighted mean relative to unweighted mean")
lines(RMSErel~Dq,data=tasmaxRMSEagg,col="red",lwd=1)
lines(RMSErel~Dq,data=tasminRMSEagg,col="blue",lwd=1)
lines(RMSErel~Dq,data=prRMSEagg,col="green",lwd=1)
abline(v=0.7,lty=3)
legend("topright",c("Multivariate Mean","tasmax","tasmin","pr"),col=c("black","red","blue","green"),lwd=c(3,1,1,1))
