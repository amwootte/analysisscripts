...
if(varname == "heatwaves"){varin="tmmx"; dataunits1="days"}
...
for(y in 1:length(years)){
    ...
    if(varname=="heatwaves"){ 
      ...
      test = nc_open(paste("/data4/data/DS_proj/METDATA/",varin,"_",years[y],".nc",sep=""))
      vardata1 = ncvar_get(test,test$var[[1]]$name,start=c(latidx[1],lonidx[1],1),count=c(length(latidx),length(lonidx),-1))
      nc_close(test)
      test = nc_open(paste("/data4/data/DS_proj/METDATA/tmmn_",years[y],".nc",sep=""))
      vardata2 = ncvar_get(test,test$var[[1]]$name,start=c(latidx[1],lonidx[1],1),count=c(length(latidx),length(lonidx),-1))
      nc_close(test)
      ...
      for(r in 1:loccount[1]){
        for(c in 1:loccount[2]){
          yearlydat[r,c,y] = heatwave.calc(vardata1[r,c,],vardata2[r,c,],q95tmax[r,c],q95tmin[r,c])
