...
W = WA = array(NA,dim=c(nummodels,length(Dcvec)))
...
weightedmeanarray = array(0,dim=c(length(lon),length(lat),12,length(Dcvec)))
...
for(i in 1:length(obsidx)){
  ...
  for(c in 1:length(Dcvec)){
    ...
    RMSEval = RMSE(exp=moddat,obs=obsdat)
    frametmp = data.frame(obs,Dc,RMSEval)
...
if(i==1){
  plot(RMSErel~Dc,data=tmpframe,type="l",lwd=2,col="black",ylim=range(tasmaxweightedRMSE$RMSErel),main="Tasmax RMSE scores",ylab="RMSE score of weighted mean relative to unweighted mean")
} else {
  lines(RMSErel~Dc,data=tmpframe,col=cols[i],lwd=2)
}
...
for(i in 1:5){
  ...
  if(i==1){
    plot(RMSErel~Dc,data=tmpframe,type="l",lwd=2,col="black",main="Tasmin RMSE scores",ylim=range(tasminweightedRMSE$RMSErel),ylab="RMSE score of weighted mean relative to unweighted mean")
  } else {
    lines(RMSErel~Dc,data=tmpframe,col=cols[i],lwd=2)
  }
...
for(i in 1:5){
  ...
  if(i==1){
    plot(RMSErel~Dc,data=tmpframe,type="l",lwd=2,col="black",main="Pr RMSE scores",ylim=range(prweightedRMSE$RMSErel),ylab="RMSE score of weighted mean relative to unweighted mean")
  } else {
    lines(RMSErel~Dc,data=tmpframe,col=cols[i],lwd=2)
  }
...
for(c in 1:length(Dcvec)){
  ...
  tmp = subset(tasmaxweightedRMSE,Dc==Dcvec[c])
  tmaxtmp = apply(tmp[,3:4],2,mean,na.rm=TRUE)
  RMSEval = tmaxtmp[1]
  RMSErel = tmaxtmp[2]
  tmpframe = data.frame(Dc,RMSEval,RMSErel)
  tasmaxRMSEagg = rbind(tasmaxRMSEagg,tmpframe)
  
  tmp = subset(tasminweightedRMSE,Dc==Dcvec[c])
  tmaxtmp = apply(tmp[,3:4],2,mean,na.rm=TRUE)
  RMSEval = tmaxtmp[1]
  RMSErel = tmaxtmp[2]
  tmpframe = data.frame(Dc,RMSEval,RMSErel)
  ...
  tmp = subset(prweightedRMSE,Dc==Dcvec[c])
  tmaxtmp = apply(tmp[,3:4],2,mean,na.rm=TRUE)
  RMSEval = tmaxtmp[1]
  RMSErel = tmaxtmp[2]
  tmpframe = data.frame(Dc,RMSEval,RMSErel)
...
RMSEagg2 = aggregate(RMSEagg$RMSErel,by=list(Dc=RMSEagg$Dc),mean,na.rm=TRUE)

plot(x~Dc,data=RMSEagg2,type="l",lwd=3,col="black",main="All RMSE scores by Dc radius",ylim=range(c(prRMSEagg$RMSErel,tasmaxRMSEagg$RMSErel,tasminRMSEagg$RMSErel)),ylab="RMSE score of weighted mean relative to unweighted mean")
lines(RMSErel~Dc,data=tasmaxRMSEagg,col="red",lwd=1)
lines(RMSErel~Dc,data=tasminRMSEagg,col="blue",lwd=1)
lines(RMSErel~Dc,data=prRMSEagg,col="green",lwd=1)
abline(v=0.7,lty=3)
