...
for(i in 1:length(GCMs)){
  ...
  for(f in 1:length(filesused)){ #
    ...
    if(var=="pr"){
      ...
      for(y in 1:length(ymon)){
        ...
        if(varout=="rx1day"){
          tmp1 = apply(testdat[,,idxuse],c(1,2),max,na.rm=TRUE)
        }
        
        if(varout=="rx5day"){
          tmp1 = apply(testdat[,,idxuse],c(1,2),calcrollsum,size=5)
        }
        testdatout[,,I]=ifelse(is.na(testdat[,,1])==TRUE,NA,tmp1)
        I=I+1
      }
  ...
  if(tempres=="monthly"){
    filenameout = paste("/home/woot0002/RCMES/data/LOCA/",varout2,"_mon_",futureperiod[1],"-",futureperiod[2],"_CONUS.nc",sep="")
  }
