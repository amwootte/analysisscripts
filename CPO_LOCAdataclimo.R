...
oddGCMs = c("MIROC5","MRI-CGCM3","MPI-ESM-LR","MPI-ESM-MR","NorESM1-M","bcc-csm1-1-m")
...
for(i in 1:length(oddGCMs)){
  ...
  filelist = system(paste("ls ",filepath,"tasmin/historical/*historical_1980.nc",sep=""),intern=TRUE)
  ...
  for(h in 1:length(histmonths)){
    ...
    dates = seq(as.Date(paste(substr(histmonths[h],1,4),"-01-01",sep="")),as.Date(paste(substr(histmonths[h],1,4),"-12-31")),by="day")
    
    nctest = nc_open(filelist[fileidx])
    times = ncvar_get(nctest,"time")
    if(length(times)<length(dates)) dates = dates[-which(substr(dates,6,10)=="02-29")]
    dateidx = which(substr(dates,6,7)==substr(histmonths[h],6,7))
    tmin = ncvar_get(nctest,paste("tasmin_",split2o[f,3],"_",split2o[f,4],"_historical",sep=""),start=c(lonidx[1],latidx[1],dateidx[1]),count=c(length(lonidx),length(latidx),length(dateidx)))
    nc_close(nctest)
    tmin32 = ifelse(tmin<=0,1,0)
  ...
  for(month in 1:12){
    monidx = which(as.numeric(substr(histmonths,6,7))==month)
    tminclimo[,,month] = apply(TMIN[,,monidx],c(1,2),mean,na.rm=TRUE)
    tmin325climo[,,month] = apply(TMIN32[,,monidx],c(1,2),mean,na.rm=TRUE)
    message("Finished climo calcs for month ",month," and file ",f)
  }
  
  TMINlist[[(startpoint+f)]] = tminclimo
  TMIN32list[[(startpoint+f)]] = tmin32climo
  
  rm(TMIN)
  rm(TMIN32)
  rm(tminclimo)
  rm(tmin32climo)
  message("Finished calcs for GCM ",oddGCMs[f])
}
...
for(f in 1:length(filelistpr1)){
  ...
  for(h in 1:length(histmonths)){
    ...
    PR50[,,h] = ifelse(is.na(TMAXlist[[1]][,,1])==FALSE,PR50[,,h],NA)
    message("Finished calcs for histmonth ",histmonths[h]," and file ",f)
  }
