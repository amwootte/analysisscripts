...
DM=3
filesin = system(paste("ls /data4/data/DS_proj/LOCA/DM",DM,"/historical/*.nc",sep=""),intern=TRUE)
...
for(i in 1:length(filesin)){
  ...
  start_time <- Sys.time()
  DMout = apply(testdat,c(1,2),sum,na.rm=TRUE)/length(unique(substr(dates[yearidx],1,4)))
...
filesin = system(paste("ls /data4/data/DS_proj/LOCA/DM",DM,"/future/*.nc",sep=""),intern=TRUE)
...
for(i in 1:length(filesin)){
  ...
  start_time <- Sys.time()
  DMout = apply(testdat[,,fileidx],c(1,2),sum,na.rm=TRUE)/length(futureperiod[1]:futureperiod[2])
  DMdata[,,i] = ifelse(is.na(testdat[,,1])==FALSE,DMout,NA)
  end_time <- Sys.time()
  end_time - start_time
...
rangeout = range(c(rcp45meanchange[idxlon,],rcp85meanchange[idxlon,])*30,na.rm=TRUE)
absoutmax = max(ceiling(abs(rangeout)))
rangeuse = c(-absoutmax,absoutmax)

diffcolorbar = colorramp(c(rcp45meanchange,rcp85meanchange)*30,colorchoice="greentobrown",Blimit=20,use_fixed_scale = TRUE,fixed_scale=rangeuse,type="difference") #,fixed_scale=fixed_scale

pdf(paste("/home/woot0002/PSS_DM",DM,"_",futureperiod[1],"-",futureperiod[2],".pdf",sep=""),width=10,height=7)

par(mfrow=c(1,2))

testsfc = list(x=lon-360,y=lat,z=rcp45meanchange*30)
surface(testsfc,type="I",xlim=c(-113,-88),main=paste(futureperiod[1],"-",futureperiod[2]," RCP 4.5\n Mean Projected Change D",DM," conditions",sep=""),zlim=diffcolorbar[[1]],col=diffcolorbar[[3]],breaks=diffcolorbar[[2]],xlab="Longitude",ylab="Latitude")
map("world",add=TRUE) # 
...
testsfc = list(x=lon-360,y=lat,z=rcp85meanchange*30)
...
meanval45[j]=mean(testin,na.rm=TRUE)*30

testin = matrix(getValues(mod.subV85[[j]]),nrow=length(lon),ncol=length(lat))
testin = testin[,length(lat):1]
meanval85[j]=mean(testin,na.rm=TRUE)*30
}
...
for(j in 1:6){
  ...
  sdval45[j] = sd(modvals45*30,na.rm=TRUE)
  sdval85[j] = sd(modvals85*30,na.rm=TRUE)
}
...
ggplot(dataindat, aes(x=scen, y=meanval, fill=scen)) + 
  ...
  ggtitle(paste(futureperiod[1],"-",futureperiod[2]," Projected Change in D",DM," conditions",sep=""))+xlab("scenario")+ylab("Projected Change (mon/year)")
ggsave(paste("/home/woot0002/PSS_barplot_DM",DM,"_",futureperiod[1],"-",futureperiod[2],".pdf",sep=""),width=10,height=5)
