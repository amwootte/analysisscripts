...
varname = "tmax100" # short name for the variable of interest options include tasmax, tasmin, pr, tmax95, tmax100, tmin32, tmin28, pr25, and pr50
...
applymask = "new mexico" # if you don't want to apply a state mask, leave this alone
...
TH = 310.928  # Threshold value - what's the threshold the script should calculate for?
...
for(i in 1:length(projfilelist)){
  ...
  yearlyoutput = netcdftoyearlycalcs(projfilelist[i],varname=varin,dimnames=c("lon","lat","time"),threscalc=TC,thres=TH,condition=cond,yearlydataperiod=c(2041,2070),datesdataperiod=datesin,appliedfunction=appfunc)
...
#############
# Calculate range for area

diffsg1range = range(diffsg1[[3]],na.rm=TRUE)
diffsg1range
